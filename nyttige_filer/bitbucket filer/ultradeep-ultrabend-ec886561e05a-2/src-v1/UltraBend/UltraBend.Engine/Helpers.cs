...
namespace UltraBend
{
    public static class Helpers
    {
        ...
        public static Task RunProcessAsync(string fileName, string workingDirectory, string arguments, OutputsService output, bool printOutput = false)
        {
            if (printOutput)
            {
                output?.PublishLineToChannel("Engine", $"File name: {fileName}");
                output?.PublishLineToChannel("Engine", $"Working directory: {workingDirectory}");
                output?.PublishLineToChannel("Engine", $"Arguments: {arguments}");
            }
            ...
            process.Exited += async (sender, args) =>
            {
                if (printOutput)
                {
                    ...
                    output?.PublishLineToChannel("Engine", $"{standardError}");
                    output?.PublishLineToChannel("Engine", $"{standardOutput}");
                }
