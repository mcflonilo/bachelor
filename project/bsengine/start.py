        ...
        for j in OD:
            ...
            for k in CL:
                ...
                for l in MAT:
                    ...
                    q = MATID[x]
                    x = x + 1
                    inp = open("Case" + str(y) + '-' + str(j) + '-' + str(k) + '-' + str(q) + ".inp", 'w')
...
def run_exe():
    try:
        generate_case_files(
            ...
            cases_field.get().split(', '), float(id_field.get()), float(sl_field.get()), od_field.get().split(', '),
            cl_field.get().split(', '), float(tl_field.get()), float(tod_field.get()), mat_field.get().split(', '),
        ...
        for case in cases:
            process = subprocess.Popen(f".\\bsengine -b {case}", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, cwd=current_directory)  # Use `ls` on Linux/Mac
...
button2 = tk.Button(root, text="Generate Case Files", command=lambda: generate_case_files(
    float(length_field.get()), float(ea_field.get()), float(ei_field.get()), float(gt_field.get()), float(m_field.get()),
    cases_field.get().split(', '), float(id_field.get()), float(sl_field.get()), od_field.get().split(', '),
    cl_field.get().split(', '), float(tl_field.get()), float(tod_field.get()), mat_field.get().split(', '),
    matid_field.get().split(', ')
