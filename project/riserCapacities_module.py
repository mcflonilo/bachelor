...
class riserCapacities:
    ...
    def update_plot(self):
        ...
        curvature_normal, tension_normal = zip(*self.get_data_from_entries(self.normal_rows))
        curvature_abnormal, tension_abnormal = zip(*self.get_data_from_entries(self.abnormal_rows))
    ...
    def interpolate_and_plot(self):
        ...
        tensions = [float(entry.get()) for entry in self.interpolation_rows if entry.get().strip() != ""]
...
def main():
    ...
    app = riserCapacities(root, prev_frame)
    root.mainloop()
