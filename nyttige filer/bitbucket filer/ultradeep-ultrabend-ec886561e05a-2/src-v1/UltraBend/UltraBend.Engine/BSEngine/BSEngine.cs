...
namespace UltraBend.Engine.BSEngine
{
    public class BsEngine
    {
        private const string BsEngineFilePath = "bsengine.exe";
        private const string BsEngineLicenseFilePath = "bsengine.lic";
        ...
        public async Task RunTestAsync()
        {
            ...
            output?.PublishLineToChannel(channel, "Test Kie Engine Input Created");
        ...
        public async Task<BendStiffenerOutput> ExecuteAsync(BendStiffenerConfiguration config)
        {
            // get the path to run in
            var path = GetPath(config);
            
            output?.PublishLineToChannel(channel, $"Execution Path: {path}");
            ...
            output?.PublishLineToChannel(channel, $"Output Path: {outputPath}");
            ...
            {
                output?.PublishLineToChannel(channel, $"Error: Output file, {outputPath}, did not exist.");

                return null;
            }
        }

        public async Task<BendStiffenerOutput> ParseOutput(string path, string pathFea)
        {
            var bendStiffenerOutput = new BendStiffenerOutput();

            using (TextReader reader = File.OpenText(path))
            {
                await bendStiffenerOutput.ReadLogAsync(reader, output, channel);
            }
